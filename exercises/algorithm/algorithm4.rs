    ...
    T: Ord,
{
    ...
    fn insert(&mut self, value: T) {
        match self.root {
            None => {
                self.root = Some(Box::new(TreeNode::new(value))); // 添加根节点
            }
            Some(ref mut node) => {
                node.insert(value); // 调用 TreeNode 的 insert 方法
            }
        }
    }
        ...
        match value.cmp(&self.value) {
            Ordering::Less => {
                if let Some(ref mut left_child) = self.left {
                    ...
                    self.left = Some(Box::new(TreeNode::new(value))); // 在左侧创建新节点
                }
            }
            Ordering::Greater => {
                if let Some(ref mut right_child) = self.right {
                    ...
                    self.right = Some(Box::new(TreeNode::new(value))); // 在右侧创建新节点
                }
            }
            Ordering::Equal => {
                // 如果相等则不做任何操作，保持树中不出现重复元素
            }
        }
    }
    fn search(&self, value: T) -> bool {
